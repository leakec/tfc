...
p1.ax[0].plot(dif['C'],dif['time'],'ko', label='Differential Corrector', markersize = MS)
...
p3.ax[0].plot(tfc['C'],tfcRes,'rx', label='TFC', markersize = MS)
p3.ax[0].plot(dif['C'],dif['res'],'ko', label='Differential Corrector', markersize = MS)
...
p4 = MakePlot('x','y')
p4.ax[0].plot(L1,0.,'ko', markersize=2)
p4.ax[0].plot(L2,0.,'ko', markersize=2)
p4.ax[0].plot(1.-mu,0.,'ko', markersize=6)

for i in range(len(tfc['C'])):
    line1 = p4.ax[0].plot(tfc['sol'][:,0,i],tfc['sol'][:,1,i],'b')
p4.ax[0].grid(True)
