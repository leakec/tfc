""" Use the TFC method to solve for balloon shapes at all altitudes using
    a fixed gore length and sig_c = constant. """
...
c = lambda xi,const: 2./(const['ld']-s0(xi))
s = lambda x,xi,const: (x+1.)/c(xi,const)+s0(xi)
...
resTh = lambda x,xi,const: c(xi,const)*dth(x,xi)-q(x,xi,const)*xi['sigC']*np.cos(th(x,xi))+q(x,xi,const)*r(x,xi)*(w*np.sin(th(x,xi))+const['b']*(z(x,xi)-z0(xi)))
resQ = lambda x,xi,const: c(xi,const)*dq(x,xi,const)+q(x,xi,const)**2*(xi['sigC']*np.sin(th(x,xi))+r(x,xi)*w*np.cos(th(x,xi)))
resR = lambda x,xi,const: c(xi,const)*dr(x,xi)-np.sin(th(x,xi))
resZ = lambda x,xi,const: c(xi,const)*dz(x,xi)-np.cos(th(x,xi))
res = jit(lambda xi,const: np.hstack([resTh(x,xi,const), resQ(x,xi,const), resR(x,xi,const), resZ(x,xi,const)]))
...
const = {'g':0.,'L':0.,'atm_density':0.,'Msg':0.,'b':0.,'ld':ld}
...
for k in range(atmData.shape[0]-1,-1,-1):
    ...
    if k == atmData.shape[0]-1:
        ...
        xi = TFCDict({'xiTh':onp.zeros((m)), 'xiQ':onp.zeros((m1)), 'xiR':onp.zeros((m)),
                      'xiZ':onp.zeros((m1)), 'beta':onp.zeros((1)), 'sigC':onp.zeros((1))})
