# This script solves the simple boundary-value problem with spectral method (Appendix B) 
# in the TFC book
####################################################################################################
...
y0  = 0.    # y(0)  = 0
yf  = 0.    # y(pi) = 0
nC  = 0     # number of constraints, we are solvoing this with spectral method
...
L0      = lambda xi: y(x,xi)[0] - y0
Lf      = lambda xi: y(x,xi)[-1] - yf
Ldif    = lambda xi: ypp(x,xi) + y(x,xi)*yp(x,xi) - f(x)

L = lambda xi: np.hstack(( L0(xi), Ldif(xi)[1:-1], Lf(xi) ))
